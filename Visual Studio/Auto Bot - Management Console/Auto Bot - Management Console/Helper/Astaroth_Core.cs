...
namespace Astaroth_Core
{
    public class Astaroth_Core
    {
        ...
        public void debug_ss()
        {
            try
            {
                if (Directory.Exists(Application.StartupPath + @"\debug_ss") == false)
                    Directory.CreateDirectory(Application.StartupPath + @"\debug_ss");
                ...
                bmp.Save(Application.StartupPath + @"\debug_ss\" + Main_Form_Init.debug_ss + ".png", ImageFormat.Png);
                bmp.Dispose();
            }
            catch (Exception ex)
            {
                Logging.Logging.log_error("Astaroth Core", "Debug SS", ex.Message);
            }
        }
        ...
        public static Rectangle FindImageOnScreen(Bitmap bmpMatch, bool ExactMatch)
        {
            ...
            try
            {
                ...
                Bitmap ScreenBmp = new Bitmap(Application.StartupPath + @"\ss\" + Main_Form_Init.ss + ".png");
        ...
        public static void take_ss()
        {
            try
            {
                ...
                Graphics g = Graphics.FromImage(bmp);
                g.CopyFromScreen(rect1.Left, rect1.Top, 0, 0, bmp.Size, CopyPixelOperation.SourceCopy);
                bmp.Save(Application.StartupPath + @"\ss\" + Main_Form_Init.ss + ".png", ImageFormat.Png);
                bmp.Dispose();
            }
            catch (Exception ex)
            {
